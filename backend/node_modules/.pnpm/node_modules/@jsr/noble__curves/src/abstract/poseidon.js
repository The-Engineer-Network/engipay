 ...
 * @module
 */ /*! noble-curves - MIT License (c) 2022 Paul Miller (paulmillr.com) */ import { asafenumber, bitGet, validateObject } from "../utils.js";
...
export function grainGenConstants(opts, skipMDS = 0) {
  ...
  for(let i = 0; i < t; i++){
    ...
    for(let j = 0; j < t; j++){
      ...
      if (Fp.is0(xy)) throw new Error(`Error generating MDS matrix: xs[${i}] + ys[${j}] resulted in zero.`);
      row.push(xy);
    }
...
export function validateOpts(opts) {
  ...
  const _mds = mds.map((mdsRow)=>{
    if (!Array.isArray(mdsRow) || mdsRow.length !== t) throw new Error('invalid MDS matrix row: ' + mdsRow);
    return mdsRow.map((i)=>{
      if (typeof i !== 'bigint') throw new Error('invalid MDS matrix bigint: ' + i);
      return Fp.create(i);
    });
  });
  if (rev !== undefined && typeof rev !== 'boolean') throw new Error('invalid param reversePartialPowIdx=' + rev);
  if (roundsFull & 1) throw new Error('roundsFull is not even' + roundsFull);
  const rounds = roundsFull + roundsPartial;
  if (!Array.isArray(rc) || rc.length !== rounds) throw new Error('Poseidon: invalid round constants');
  ...
  if (!sboxPower || ![
    ...
    7,
    17
  ].includes(sboxPower)) throw new Error('invalid sboxPower');
...
/** Poseidon NTT-friendly hash. */ export function poseidon(opts) {
  ...
  const poseidonHash = function poseidonHash(values) {
    if (!Array.isArray(values) || values.length !== t) throw new Error('invalid values, expected array of bigints with length ' + t);
    values = values.map((i)=>{
...
export class PoseidonSponge {
  ...
  absorb(input) {
    for (const i of input)if (typeof i !== 'bigint' || !this.Fp.isValid(i)) throw new Error('invalid input: ' + i);
