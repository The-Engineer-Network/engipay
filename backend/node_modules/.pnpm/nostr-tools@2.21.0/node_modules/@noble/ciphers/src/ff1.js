...
function getRound(radix, key, tweak, x) {
  ...
  if (2 > minLen || minLen > maxLen || maxLen >= 2 ** 32) throw new Error('Invalid radix: 2 ≤ minlen ≤ maxlen < 2**32');
...
/** FPE-FF1 format-preserving encryption */ export function FF1(radix, key, tweak = EMPTY_BUF) {
  anumber(radix);
  abytes(key);
  abytes(tweak);
...
/** Binary version of FPE-FF1 format-preserving encryption. */ export function BinaryFF1(key, tweak = EMPTY_BUF) {
