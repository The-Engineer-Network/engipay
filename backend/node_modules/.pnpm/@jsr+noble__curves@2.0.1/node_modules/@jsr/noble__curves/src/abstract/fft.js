...
/** Checks if integer is in form of `1 << X` */ export function isPowerOfTwo(x) {
  checkU32(x);
  return (x & x - 1) === 0 && x !== 0;
}
...
/** Similar to `bitLen(x)-1` but much faster for small integers, like indices */ export function log2(n) {
  checkU32(n);
 ...
 */ export function bitReversalInplace(values) {
  ...
  if (n < 2 || !isPowerOfTwo(n)) throw new Error('n must be a power of 2 and greater than 1. Got ' + n);
  const bits = log2(n);
  for(let i = 0; i < n; i++){
...
/** We limit roots up to 2**31, which is a lot: 2-billion polynomimal should be rare. */ export function rootsOfUnity(field, generator) {
